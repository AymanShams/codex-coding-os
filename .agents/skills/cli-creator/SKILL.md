---
name: cli-creator
description: "Use when the user asks to build, scaffold, design, harden, install, or improve a durable command-line tool or CLI for Codex from API docs, an OpenAPI spec, curl examples, SDK docs, a web app, an admin tool, existing scripts, shell history, logs, or repeated operational commands. Use when the tool should run from any repo, expose composable commands, return stable JSON, manage auth/config, support doctor/help commands, provide read/write verbs, and pair with a companion skill. Do not use for one-off scripts inside the current repo, normal app features, plugin creation, skill creation, MCP server creation, generic shell command help, or existing CLI usage without new CLI design. If the primary task is creating or editing a Codex skill, use skill-creator. If the primary task is scaffolding a Codex plugin, use plugin-creator. If the primary task is a web app or UI, use frontend or build-web-app skills."
---
