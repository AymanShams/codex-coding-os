---
name: process-docs
description: Use when the user asks to document, standardize, explain, or turn a recurring internal process into a lightweight operational SOP, playbook, runbook, onboarding playbook, checklist, procedure, or team guide that helps someone execute the process consistently. Use for prompts like write a simple SOP, document this process, create a runbook, build an onboarding playbook, standardize this workflow, make this repeatable, capture tribal knowledge, define handoffs, add decision points, or write step-by-step instructions for a routine process. Prefer this for practical internal process documentation when the artifact is not legal-facing, audit-facing, board-facing, governance-controlled, or intended as a formal source of truth. Do not use for customer-facing help center content, API docs, broad artifact-system design, WBS planning, casual notes, or formal controlled SOPs, policies, operating models, governance artifacts, RACI or DACI matrices, implementation guides, legal or contract drafts, and company source-of-truth artifacts. If the primary task is a controlled or high-stakes artifact, use ssot-drafter. If the primary task is documenting the surrounding document library, folder system, metadata, or lifecycle, use artifact-system-designer. If the primary task is a delivery plan, issue tree, roadmap, or dependency plan, use wbs-artifact-planner. If the primary task is customer-facing support documentation, use support-docs.
