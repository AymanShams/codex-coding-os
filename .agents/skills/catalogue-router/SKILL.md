...
- Worker execution requires `task_input_mode=complete`, `issuance.status=registered`, registry schema version 3, `failure_code=null`, every recipe input requirement, both matching task hashes, matching current manifest and policy authority hashes, and an exact unexpired receipt. A manifest or policy change invalidates the receipt even when its snapshot label was reused. The raw hook's `conservative_instruction_only` route can advise Codex but cannot run a worker.
