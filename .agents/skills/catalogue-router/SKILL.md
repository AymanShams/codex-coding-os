...
Optional `references/capability-catalogue.md` is historical discovery evidence only. It must not select a route or compete with the canonical policy. The router authority transaction owns this skill body, its query wrapper, and that evidence file, so ordinary package installation cannot replace or remove the live router contract.
...
- Worker execution requires `task_input_mode=complete`, `issuance.status=registered`, registry schema version 3, `failure_code=null`, every recipe input requirement, both matching task hashes, matching current manifest and policy authority hashes, and an exact unexpired receipt. Manifest schema 1.3 must resolve through the current immutable generation pointer. A manifest or policy change invalidates the receipt even when its snapshot label was reused. The raw hook's `conservative_instruction_only` route can advise Codex but cannot run a worker.
- Registry canonical JSON uses `json.dumps` with `ensure_ascii=False`, `sort_keys=True`, `separators=(",", ":")`, and `allow_nan=False`, encoded as UTF-8. The WAL registry stores the route, both task hashes, both current-authority hashes, snapshots, and issuance times, but no prompt or task input. Migration from an obsolete schema atomically purges old receipts. It never prunes an unexpired issuance. At 10,000 simultaneous unexpired routes, new issuance fails closed. Count and age pruning apply only to expired audit rows.
...
- A capability with `requires_live_dependencies` is selectable only when the current immutable manifest generation contains the exact dependency entry, the entry is not quarantined, and the current config value proves it enabled. A schema 1.3 manifest may remain usable in a scoped `degraded` state only for capabilities outside a proven package, app-surface, or worker-runtime quarantine. A dated catalogue, installed plugin root, skill file, or stale tool-family row is not dependency evidence. If the dependency closure is unreadable, malformed, mixed, or unprovable, fail the complete authority closed.
- Gateway-managed workers also require the generation-bound worker runtime bill of materials to match the configured source root, exact identity-file bytes, release identifier, route schema 3.0, and registry schema 3. Router admission and worker runtime status are separate results.
