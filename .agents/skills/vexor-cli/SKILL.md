---
name: vexor-cli
description: "Use when the user asks to locate, discover, map, or find where something is implemented, loaded, defined, documented, configured, routed, or referenced in a medium or large repository and the file location is unclear. Use semantic file discovery via `vexor` for intent-based searches, unfamiliar codebases, broad feature location, architecture entrypoints, docs sections, config loaders, command definitions, and file discovery before deeper inspection. Do not use when the exact filename or text pattern is already known, for small obvious repos, for simple `rg` text searches, for non-repository web research, or for editing files after discovery. If the primary task is graph-backed dependency impact or blast-radius analysis, use code-review-graph. If the primary task is project-docs-backed questioning, use grill-with-docs."
---
