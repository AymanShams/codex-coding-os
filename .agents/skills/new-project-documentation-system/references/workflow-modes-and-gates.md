...
Parent-orchestrator mode and automatic session, review, and review-fix trains are disabled. A human must deliberately start every bounded session. Before review, record one stable case ID. Use one independent review, one human-authorized combined repair for current blockers, and one final blocker-closure check. Findings are `current_blocker`, `non_blocking`, `invalid_or_stale`, or `redesign_required`. Any remaining or new blocker, failed validation, expanded scope, or redesign requirement permanently marks the case `RED_LOCKED`. Renaming, branching, opening another pull request, changing chats or agents, editing counters, running root-cause analysis, or adding an adversarial matrix cannot reset the case or authorize another review. Only a separate human-led decision may start a materially different design from clean `main` under a new case ID.
