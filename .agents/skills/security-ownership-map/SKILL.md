...
description: "Use when the user asks for security-oriented ownership analysis, bus-factor analysis, sensitive-code ownership, orphaned sensitive code, CODEOWNERS reality checks for risk, hidden maintainers, security maintainer mapping, sensitive hotspots, ownership clusters, co-change ownership risk, or people-to-file topology grounded in git history. Use to analyze git repositories and export CSV, JSON, GraphML, or graph artifacts for security ownership visualization. Do not use for general maintainer lists, generic team ownership, normal git blame, non-security ownership questions, broad code review, threat modeling, secure coding best practices, or dependency impact analysis. If the primary task is AppSec threat modeling, use security-threat-model. If the primary task is language/framework security guidance or a vulnerability report, use security-best-practices. If the primary task is graph-backed code dependency impact, use code-review-graph."
---
