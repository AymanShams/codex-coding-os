...
Before a parent/orchestrator final closeout, re-check the exact PR head, current-head review records, inline comments, issue comments, required checks, local branch, local HEAD, and working-tree state. GitHub collection and `scripts/agent/session_continuity.py` report raw facts only. A `COMMENTED` review, issue-comment prose, a clean-sounding summary, coordination manifest, or handoff cannot approve or close a case. Typed validation evidence records what a check proves and does not prove, but it does not own lifecycle.
